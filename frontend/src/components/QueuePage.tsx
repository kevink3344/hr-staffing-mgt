...
export function QueuePage({ onBack }: QueuePageProps) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-mono">
            <header className="bg-white border-b-4 border-gray-300 p-6">
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <select
                            ...
                            className="border-2 border-gray-800 rounded-2px px-3 py-1.5 font-mono text-sm bg-white"
                        >
                        ...
                        <span className="text-sm text-gray-500 ml-2">{items.length} item{items.length !== 1 ? 's' : ''}</span>
                    </div>
                </div>
            </header>
            ...
            <main className="max-w-6xl mx-auto p-6">
                {isLoading ? (
                    ...
                    <div className="overflow-x-auto border-2 border-gray-800 rounded-2px">
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {items.map((item, idx) => (
                                    <tr key={item.id} className={`border-t border-gray-300 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-gray-100`}>
                                        <td className="px-3 py-2 text-gray-500">{idx + 1}</td>
                                        {editingId === item.id ? (
                                            <>
                                                <td className="px-3 py-2">
                                                    <input value={editForm.employee_name || ''} onChange={(e) => setEditForm({ ...editForm, employee_name: e.target.value })}
                                                        className="border border-gray-400 rounded-2px px-2 py-1 w-full font-mono text-sm" />
                                                </td>
                                                <td className="px-3 py-2">
                                                ...
                                                <td className="px-3 py-2">
                                                    <input type="date" value={editForm.effective_date || ''} onChange={(e) => setEditForm({ ...editForm, effective_date: e.target.value })}
                                                        className="border border-gray-400 rounded-2px px-2 py-1 font-mono text-sm" />
                                                </td>
                                            </>
                                        ) : (
                                            <>
                                        ...
                                        <td className="px-3 py-2">
                                            <div className="flex gap-1">
                                                {editingId === item.id ? (
                                                    ...
                                                    <>
                                                        <button onClick={() => startEdit(item)} className="px-2 py-1 text-xs font-bold text-blue-700 hover:text-blue-900 border border-blue-300 rounded-2px hover:bg-blue-50">
                                                            Edit
                                                        </button>
